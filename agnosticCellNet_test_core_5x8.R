...
# Siwei 15 Sept 2023
# Use the core 5x8 including iPS data from May 2021 for testing
...
# load core 5x8 count matrix from RData
df_core_5x8 <-
  readRDS("core_5x8_raw_reads_Geneid.RDs")
...
df_core_5x8 <-
  merge(x = df_core_5x8,
        ...
        by = "Geneid")
df_core_5x8 <-
  df_core_5x8[!duplicated(df_core_5x8$Gene_Symbol), ]
# Keep gene name as a separate vector
gene_name_list <-
  df_core_5x8$Gene_Symbol
df_queryExpDat <-
  df_core_5x8
# remove all non-numerical columns
df_queryExpDat$Geneid <- NULL
df_queryExpDat$Gene_Symbol <- NULL
...
df_querySampTab <-
  data.frame(sample_name = colnames(df_queryExpDat),
             pe_se = "PAIRED",
             description1 = str_split(string = colnames(df_queryExpDat),
                                      pattern = "_",
                                      simplify = T)[, 1],
             description2 = "core_5x8_samples",
             stringsAsFactors = F)
...
saveRDS(my_classifier,
     file = "core_5x8/cellnet_classifier_100topGenes_100genePairs.rda")
...
saveRDS(xpairs_list,
     file = "core_5x8/Hs_xpairs_list.rda")
...
save(classMatrixQuery,
     file = "core_5x8/example_classificationMatrix.rda")
